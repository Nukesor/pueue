    ...
    async fn handle_complex_command(&mut self, subcommand: SubCommand) -> Result<bool> {
        ...
        match subcommand {
            SubCommand::Add {
                ...
                if follow {
                    // Don't print anything when following in case the user wants to pipe the output
                    // somewhere else.
                } else if print_task_id {
                    println!("{task_id}");
                } else if let Some(enqueue_at) = enqueue_at {
                    let enqueue_at = format_datetime(&self.settings, &enqueue_at);
                    println!("New task added (id {task_id}). It will be enqueued at {enqueue_at}");
                } else {
                    println!("New task added (id {task_id}).");
                };
