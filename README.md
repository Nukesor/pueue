...
`pueue kill [keys...] -s [signal]` This command tries to copy the behaviour of the Linux `kill` command. It will send a signal (default is `sigterm`) to the specified processes.  
    ...
    Be aware that the signal will only be sent to the children of the shell process, i.e. if you send a `sigint` right after starting a `sleep 600 ; sleep 20` the `sleep 600` process will be stopped and the `sleep 20` will spawn afterwards. To send the signal to the parent shell process as well you need to add the `-a` flag.
