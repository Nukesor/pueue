...
pub async fn handle_show(settings: &Settings, socket: &mut TcpStream, message: StreamRequestMessage) -> Result<Message> {
    if message.follow || message.err {
        // The client requested streaming of stdout
        let mut handle: File;
        match get_log_file_handles(message.task_id, &settings) {
            Err(_) => {
                return Ok(create_failure_message("Couldn't find output files for task. Maybe it finished? Try `log`"))
            }
        ...
        let handle_path = if message.err {
            err_path
        } else {
            out_path
        };
        ...
        loop {
            ...
            if !handle_path.exists() {
                return Ok(create_success_message("File has gone away. The task probably just finished"));
            }
